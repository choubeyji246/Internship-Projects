...
const insertAnswer = async (req, res, next) => {
    try {
        ...
        const isUserAllowed = await validateUserId(authenticatedId, ansData[0].ques_id, next);
        if (!isUserAllowed) {
            throw new AppError(404, "You are not allowed to answer this question");
        }

        // Using Promise.all to execute all insert queries asynchronously
        await Promise.all(ansData.map(async ans => {
            const values = [ans.id, ans.body, ans.ques_id, authenticatedId];
